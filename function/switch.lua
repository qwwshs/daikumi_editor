...
function switch_draw(name)
    ...
    love.graphics.rectangle('fill',switch_objact[name].x, --开关内
    switch_objact[name].y,
    switch_objact[name].w * (loadstring("return _G."..switch_objact[name].var)() ) / switch_objact[name].level
    ,switch_objact[name].h)

    love.graphics.setColor(1,1,1,1) --开关
    love.graphics.rectangle('fill',switch_objact[name].x + switch_objact[name].w * (loadstring("return _G."..switch_objact[name].var)() ) / switch_objact[name].level, --开关边
    switch_objact[name].y,
    5
    ,switch_objact[name].h)
    ...
    for i = 1, switch_objact[name].level do

        love.graphics.rectangle('fill',switch_objact[name].x + switch_objact[name].w * i  / switch_objact[name].level, --开关边
        switch_objact[name].y,
        1
        ,switch_objact[name].h)
    end
...
function switch_wheelmoved(x,y,name) --滑轮滚动 使全体位移
    switch_objact[name].y = switch_objact[name].y + y
end
